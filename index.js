    ...
    .then(file => {
        ...
        function computeMapping(predict, type) {
            ...
            while (i < (data.length)-1) {
                if (!(data[i] in temp_mapping)) {
                    ...
                    let storeKey = new HashTable(temp_mapping[data[i]]).key
                    let next = data[i+1]
                    if (storeKey == next) {
                        let pVal = {}
        ...
        function computePrediction(array_dict, comeAfter) {
            ...
            for (let i = 0; i < array_dict.length; i++) {
                let currenKey = new HashTable(array_dict[i]).key
                let prediction = new HashTable(array_dict[i][currenKey]).key
                let predictValue = new HashTable(array_dict[i][currenKey]).value

                if (currenKey == comeAfter) {
